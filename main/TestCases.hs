...
runTwoSumTest (x : xs) (y : ys)
  | uncurry twoSumBF x == y && uncurry twoSumHash x == y =
    runTwoSumTest xs ys
  | otherwise =
    error
      ( "Input: " ++ show x
          ++ "is not match with Output: "
          ++ show y
      )
...
runIsValidParen (x : xs) (y : ys)
  | isValidParen x == y = runIsValidParen xs ys
  | otherwise =
    error
      ( "Input: " ++ show x
          ++ "is not match with Output: "
...
runMergeLists (x : xs) (y : ys)
  | uncurry mergeLists x == y && uncurry mergeLists' x == y = runMergeLists xs ys
  | otherwise =
    error
      ( "Input: " ++ show x
          ++ "is not match with Output: "
...
runSellStocks (x : xs) (y : ys)
  | maxProfit x == y && maxProfit' x == y = runSellStocks xs ys
  | otherwise =
    error
      ( "Input: " ++ show x
          ++ "is not match with Output: "
